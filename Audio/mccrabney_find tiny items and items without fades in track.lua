--[[
 * ReaScript Name: find tiny items and items without fades in track
 * Author: mccrabney
 * Licence: GPL v3
 * REAPER: 6.0
 * Extensions: None
  ...
  if isMIDI ~= true then                 -- if take is not MIDI
    ...
    if cursorPos < itemStart then       -- if itemstart is later than edit cursor
      if itemLength < 0.005 then 
        ...
        reaper.SetEditCurPos(itemStart, 1, 0)
        reaper.Undo_EndBlock('found a tiny item' , -1)
      return end      
      if fadeIn == 0 and autoFadeIn == 0 then 
        reaper.SetMediaItemSelected( item, 1 )
        reaper.SetEditCurPos(itemStart, 1, 0)
        reaper.Undo_EndBlock('found a fadeless item' , -1)
      return end
      if fadeOut == 0 and autoFadeOut == 0 then
        reaper.SetMediaItemSelected( item, 1 )
        reaper.SetEditCurPos(itemEnd, 1, 0)
        reaper.Undo_EndBlock('found a fadeless item' , -1)
      return end
