...
function X_to_pos(x)
  ...
  local cx, cy = reaper.JS_Window_ScreenToClient( track_window, x, 0)
  --cx, cy = ImGui.PointConvertNative(ctx, cx, cy, false)
  
  --reaper.ShowConsoleMsg(cx .. "\n")
  cx = cx >= 0 and cx or 0
...
function MouseInfo(x,y,p)
  ...
  --m_cx, m_cy = ImGui.PointConvertNative(ctx, m_cx, m_cy, false)
  ...
  if (LB_DOWN and not RB_DOWN) or (RB_DOWN and not LB_DOWN) then   -- LMB or RMB pressed down?
    if (mouse.last_LMB_state == false and not RB_DOWN) or (mouse.last_RMB_state == false and not LB_DOWN) then
      ...
      --if mouse.uptime and os.clock() - mouse.uptime < 0.10 then
      --  OnMouseDoubleClick()
      --end
    else
