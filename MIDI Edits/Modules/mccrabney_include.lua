--[[
 * ReaScript Name: mccrabney_include
 * Author: mccrabney
 * Licence: GPL v3
 * REAPER: 7.0
 * Extensions: None
...
function extStates()
  ...
  if loopCount > resetCursor then                               -- if idle, reset cursor
  ...
  if reaper.HasExtState(extName, 'noteHoldUpdate') then         
    ...
    --reaper.ShowConsoleMsg(lastNoteHoldNumber .. " " .. noteHoldNumber .. "\n")
  ...
  if reaper.HasExtState(extName, 'toggleNoteHold') then         -- if extstate says we are holding a note,
    ...
    or toggleNoteHold == 1 and lastTargetPitch ~= targetPitch then -- if RE exists but notehold is 0
