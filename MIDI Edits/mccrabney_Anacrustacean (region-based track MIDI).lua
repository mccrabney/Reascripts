      ...
      if reaper.TakeIsMIDI(take) then  
        if tonumber(itemName) == j then   -- if MIDI item name matches the region
          ...
          if math.floor(itemPos*1000) ~= math.floor(ogRgnTable[j][1]*1000) then  -- if itempos has changed from OG
            ...
            reaper.SetMediaItemSelected(item, true)  -- select items for gluing
            reaper.Main_OnCommand(40362, 0)          -- glue all selected items
            item = reaper.GetSelectedMediaItem(0, 0)                        -- update
            take = reaper.GetActiveTake(item)                               -- update
            itemPos = reaper.GetMediaItemInfo_Value(item, 'D_POSITION')     -- update
            itemLength = reaper.GetMediaItemInfo_Value(item, 'D_LENGTH')    -- update
