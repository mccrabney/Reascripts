--[[
 * ReaScript Name: move edit cursor to next MIDI note on track
 * Author: mccrabney
 * Licence: GPL v3
 * REAPER: 6.0
 * Extensions: None
...
if editCursorPos < noteStartPosTable[1] and cursNote == nil then     -- if edit cursor is before first note
  reaper.SetEditCurPos(noteStartPosTable[1], 1, 0 )  -- set edit cursor to first note pos
  cursNote = 0 
end
...
if cursNote ~= nil and cursNote + direction <= tallyNotes  then -- if cursor is in note and movement won't > total of notes,
  ...
  reaper.SetEditCurPos(noteStartPosTable[cursNote + direction], 1, 0 )  -- set edit cursor to next note pos
end

reaper.PreventUIRefresh(-1)  
reaper.Undo_EndBlock("move edit cursor to next note" ,-1)
